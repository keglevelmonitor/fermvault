#!/bin/bash
# update.sh
# Handles checking, pulling code, and dependency updates for FermVault.
...
DESKTOP_FILE="$HOME/.local/share/applications/fermvault.desktop"
if [ -f "$ICON_SOURCE" ]; then
    ...
    if [ -f "$DESKTOP_FILE" ]; then
        ...
        if ! grep -q "^StartupWMClass=" "$DESKTOP_FILE"; then
            echo "StartupWMClass=FermVault" >> "$DESKTOP_FILE"
        fi
    fi
fi
