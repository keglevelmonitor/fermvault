...
class NotificationManager:
    def __init__(self, settings_manager, ui_manager):
        ...
        # FIX: Initialize to current time to prevent instant firing on startup
        ...
        for line in lines:
            ...
            try:
                if line == "control mode ambient":
                    ...
                    value_f = self._parse_setpoint_value(value_str) 
                    
                    # Convert input C to F for storage, unless it's duration or frequency
                    ...
                    if command_key == "setpoint ambient":
                        ...
                        if new_freq < 0: raise ValueError("Frequency cannot be negative.")
    ...
    def _send_command_reply(self, recipient_email, smtp_config, results_body):
        ...
        return self._send_email_or_sms(
            subject, body, recipient_email, smtp_config, "Command Reply"
        )
    ...
    def start_scheduler(self):
        ...
        if not self._scheduler_running:
            ...
            # --- FIX: Reset conditional timer to NOW to prevent startup alerts ---
            self.last_conditional_check_time = time.time()
            
            # Handle startup logging and initial send delay
            try:
                raw_freq = self.settings_manager.get("frequency_hours", 0)
                freq_h = int(float(raw_freq))
            ...
            if freq_h > 0:
                ...
                # Schedule initial notification in ~60s
                ...
                # Set timer normally (to NOW) so it doesn't trigger if logic fails
                self.last_notification_sent_time = time.time()

            # Log Conditional Notification Status
            cond_enabled = self.settings_manager.get("conditional_enabled", False)
            if self.ui:
                status = "enabled" if cond_enabled else "disabled"
                self.ui.log_system_message(f"Conditional notifications {status}.")
            
            # Reset other timers
            self.last_api_fetch_time = time.time()
            
            # --- FIX: Schedule first FG Calc for 1 minute after startup ---
            fg_freq_h = self.settings_manager.get("fg_check_frequency_h", 24)
            fg_freq_s = fg_freq_h * 3600
            # Initialize so that (now >= last + freq) becomes true in 60 seconds
            self.last_fg_calc_time = time.time() - fg_freq_s + 60
            # --------------------------------------------------------------
    ...
    def force_reschedule(self, old_freq, new_freq):
        ...
        if self._scheduler_running:
            ...
            try:
                n_freq_safe = int(float(new_freq))
                o_freq_safe = int(float(old_freq))
            except:
                n_freq_safe = 0
                o_freq_safe = 0

            # Check if notifications were just turned ON (0 -> >0)
            if (n_freq_safe > 0) and (o_freq_safe == 0):
                # Schedule initial notification in ~60s
                interval_seconds = self._get_interval_seconds(n_freq_safe)
                self.last_notification_sent_time = time.time() + 60 - interval_seconds
                print("[NotificationManager] Scheduling initial notification in 60s.")
            else:
                # Otherwise, just reset the timer normally
                self.last_notification_sent_time = time.time()
            ...
            # FIX: Do NOT reset conditional timer here, or we risk delaying legitimate alerts.
    ...
    def _get_interval_seconds(self, frequency_hours):
        ...
        try:
            # FIX: Robust conversion. "0.0" (str) -> 0.0 (float) -> 0 (int)
            # This ensures we don't hit the exception for float strings and default to 24h
            return int(float(frequency_hours)) * HOURS_TO_SECONDS
        except (ValueError, TypeError):
            # If actual garbage data, default to 24 hours
            return 24 * HOURS_TO_SECONDS 
    ...
    def _scheduler_loop(self):
        ...
        while self._scheduler_running:
            ...
            # GUARD: Only proceed if frequency > 0
            if notif_freq_s > 0:
            ...
            # --- 4. CONDITIONAL ALERT LOGIC (Every 60 seconds) ---
    ...
    def _get_next_check_str(self):
        """Helper to format the next scheduled FG check time for the UI button."""
        try:
            fg_freq_h = self.settings_manager.get("fg_check_frequency_h", 24)
            fg_freq_s = fg_freq_h * 3600
            
            # Next run is simply Last Run + Frequency
            next_run_timestamp = self.last_fg_calc_time + fg_freq_s
            
            # Calculate local time string
            dt_local = datetime.fromtimestamp(next_run_timestamp)
    ...
    def run_fg_calc_and_update_ui(self):
        ...
        # --- FIX: Set analyzing state immediately for visual feedback ---
        self.settings_manager.set("fg_value_var", "Analyzing...")
        self.settings_manager.set("fg_status_var", "")
        
        # Force immediate UI refresh SAFELY using the adapter
        self.ui.root.after(0, self.ui._update_data_display)
        # --------------------------------------------------------------
        ...
        def fg_task():
            ...
            # --- FIX: Append Next Check time if not Stable (Clearer Format) ---
            if raw_status != "Stable":
                next_time = self._get_next_check_str()
                button_status += f"\nNext check:\n{next_time}"
            # ------------------------------------------------------------------
    ...
    def _format_message_body(self, is_status_request=False):
        ...
        amb_target = self.settings_manager.get("amb_setpoint_current", "--.-") 
        ...
        body_lines = [
            ...
            "",
            "--- Beer ---",
            f"Actual: {convert(beer_actual)}",
            f"Target: {convert(beer_set)}",
            "",
            "--- Ambient ---",
            f"Actual: {convert(amb_actual)}",
            f"Target: {convert(amb_target)}",
            f"Range: {convert(amb_min)} - {convert(amb_max)}",
            "",
            "--- Gravity ---",
            f"OG: {og_val} {og_time}",
            f"SG: {sg_val} {sg_time}",
        ...
        if results.get("stable"):
            ...
            if not has_error:
                ...
                status_msg = "Pending"
                 ...
                 # Map complex errors to simple button text if needed, or leave empty/custom
                 # For now, we use the logic from the manual method map if available, 
                 # or basic "Pending" fallback
                 status_msg = "Pending" 
            ...
            # Try to map specific error strings if they match keys
            found_key = next((k for k in SHORT_TEXT_MAP if k in log_msg), "Pending")
            status_msg = SHORT_TEXT_MAP.get(found_key, "Pending")

            # --- FIX: Append Next Check time if not Stable (Clearer Format) ---
            # Note: For scheduled run, we must calculate the *next* time manually here
            # because 'last_fg_calc_time' hasn't been updated in the scheduler loop yet.
            # The scheduler updates it immediately after this function returns.
            # So next run = Now + Freq
            
            try:
                fg_freq_h = self.settings_manager.get("fg_check_frequency_h", 24)
                fg_freq_s = fg_freq_h * 3600
                next_ts = time.time() + fg_freq_s
                dt_local = datetime.fromtimestamp(next_ts)
                next_time_str = dt_local.strftime("%H:%M")
                status_msg += f"\nNext check:\n{next_time_str}"
            except:
                pass
            # ------------------------------------------------------------------
    ...
    def _send_email_or_sms(self, subject, body, recipient_address, smtp_cfg, message_type_for_log):
        ...
        try:
            ...
            self._report_error("push", error_msg) 
            return False
    ...
    def _send_status_report(self, recipient_email, smtp_config):
        ...
        return self._send_email_or_sms(
            subject, body, recipient_email, smtp_config, "Status Request Reply"
        )

    def _send_status_message(self, is_scheduled=False):
        """Sends the periodic push notification."""
        
        try:
            raw_freq = self.settings_manager.get("frequency_hours", 0)
            notif_freq_h = int(float(raw_freq))
        except:
            notif_freq_h = 0
