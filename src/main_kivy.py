...
os.environ['SDL_VIDEO_X11_WMCLASS'] = "FermVault"
...
class KivyUIManagerAdapter:
    ...
    def _update_data_display(self):
        """
        Pulls the live, current states directly from the SettingsManager and hardware cache.
        This provides a safe, immediate UI refresh for Gravity/API updates without
        triggering a logic loop or passing empty parameters that cause UI defaults.
        """
        sm = self.app.settings_manager
        
        # Fetch current system parameters
        beer_temp = sm.get("beer_temp_actual", "--.-")
        amb_temp = sm.get("amb_temp_actual", "--.-")
        amb_min = sm.get("amb_min_setpoint", 0.0)
        amb_max = sm.get("amb_max_setpoint", 0.0)
        beer_setpoint = sm.get("beer_setpoint_current", 0.0)
        amb_target = sm.get("amb_target_setpoint", 0.0)
        current_mode = sm.get("control_mode", "Ambient Hold")
        sensor_error_message = sm.get("sensor_error_message", "")
        
        # Safely fetch live hardware states to prevent flashing
        real_heat = False
        real_cool = False
        if hasattr(self.app, 'relay_control') and self.app.relay_control:
            real_heat = self.app.relay_control.relay_state_cache.get("Heat", False)
            real_cool = self.app.relay_control.relay_state_cache.get("Cool", False)
        
        # Push the full, current dataset to the UI immediately
        self.app.push_data_update(
            beer_temp=beer_temp,
            amb_temp=amb_temp,
            amb_min=amb_min,
            amb_max=amb_max,
            beer_setpoint=beer_setpoint,
            heat_state="HEATING" if real_heat else "Heating OFF",
            cool_state="COOLING" if real_cool else "Cooling OFF",
            amb_target=amb_target,
...
class FermVaultApp(App):
    ...
    def start_backend(self, dt=None):
        if SettingsManager is None:
            ...
            # Ensures the top-most loading screen dies even if imports fail
            self.dismiss_splash()
            return
        ...
        try:
            ...
            # --- NEW: Restore Window Position/Size ---
            # We import Window here locally to ensure it is available
            ...
            # 5. Startup Logic & Persistence Check
            # --- FIX: Read saved state instead of forcing OFF ---
            saved_monitor_state = self.settings_manager.get("monitoring_state", "OFF")
            ...
            if saved_monitor_state == "ON":
                self.monitoring_state = "ON"
                self.log_system_message("PERSISTENCE: Monitoring Resumed (Auto-Start).")
                # Directly start monitoring (skips standby loop)
                self.toggle_monitoring("ON")
            else:
    ...
    def _commit_staged_changes(self):
        ...
        # --- FIX: MAPPING FOR PID DELTA KEYS (UI Property -> Backend Key) ---
        pid_key_map = {
            "ambient_mode_deadband_f": "ambient_deadband",
            "pid_envelope_f": "beer_pid_envelope_width",
            "crash_mode_envelope_f": "crash_pid_envelope_width",
            "ramp_pre_ramp_tolerance_f": "ramp_pre_ramp_tolerance",
            "ramp_thermostatic_deadband_f": "ramp_thermo_deadband",
            "ramp_pid_landing_zone_f": "ramp_pid_landing_zone"
        }
        # --------------------------------------------------------------------
        ...
        for key, val in self.staged_changes.items():
            ...
            # --- FIX: APPLY PID MAPPING ---
            elif key in pid_key_map:
                backend_key = pid_key_map[key]
                self.settings_manager.set(backend_key, val)
            # ------------------------------
    ...
    def _sync_control_mode_from_backend(self, internal_mode):
        # FIX: "OFF" is a runtime status, not a configuration mode.
        # If the backend reports "OFF" (e.g. monitoring stopped), 
        # we should NOT change the UI spinner, so the user's selected mode remains visible.
        if internal_mode == "OFF":
            return

        map_internal_to_ui = {
            "Ambient Hold": "AMBIENT", "Beer Hold": "BEER",
            "Ramp-Up": "RAMP", "Fast Crash": "CRASH"
        }
        
        # Default to current value instead of forcing AMBIENT if unknown
        ui_value = map_internal_to_ui.get(internal_mode, self.control_mode_display)
...
def run_splash_screen(queue):
    ...
    try:
        ...
        lbl = tk.Label(frame, text="FermVault App Loading...", font=("Arial", 16, "bold"), fg="#33CCFF", bg="#222222")
