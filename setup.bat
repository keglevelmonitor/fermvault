...
:: setup.bat - Bootstrap installer for FermVault (Windows)
:: Can be run from anywhere (e.g. after: curl -sL ... -o setup.bat ^&^& setup.bat)
:: Clones repo to %USERPROFILE%\fermvault and runs install.bat

SET "INSTALL_DIR=%USERPROFILE%\fermvault"
SET "DATA_DIR=%USERPROFILE%\fermvault-data"
SET "SHORTCUT_PATH=%USERPROFILE%\Desktop\FermVault.lnk"
SET "REPO_URL=https://github.com/keglevelmonitor/fermvault.git"

TITLE FermVault Auto-Installer

echo.
echo ========================================
echo    FermVault Windows Installer
echo ========================================
echo.
...
SET "WHAT_TO_INSTALL=FermVault Application and Data Directory"
...
IF /i "%choice%"=="UPDATE" (
    SET "WHAT_TO_INSTALL=FermVault Update"
    SET "CLEANUP_MODE=NONE"
    goto :confirm_proceed
)
IF /i "%choice%"=="APP" (
    SET "WHAT_TO_INSTALL=FermVault Application (Fresh App, Keep Data)"
    SET "CLEANUP_MODE=APP"
    goto :confirm_proceed
)
IF /i "%choice%"=="ALL" (
    SET "WHAT_TO_INSTALL=FermVault Application and Data Directory (Fresh Install)"
